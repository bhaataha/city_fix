...
async function seedDatabase(prismaClient) {
    const prisma = prismaClient || new client_1.PrismaClient();
    ...
    const categories = await Promise.all(categoryData.map((cat, i) => prisma.serviceCategory.upsert({
    ...
    const deptManager = await prisma.user.upsert({
        ...
        create: {
            ...
            firstName: 'דוד',
            lastName: 'לוי',
            role: client_1.UserRole.DEPT_MANAGER,
            passwordHash,
    ...
    const issueSeeds = [
        { cat: 0, desc: 'בור גדול ברחוב הרצל, מזה שבועיים', addr: 'רחוב הרצל 42', lat: 32.0649, lng: 34.7717, urgency: client_1.Urgency.HIGH, status: client_1.IssueStatus.NEW },
        { cat: 1, desc: 'פנס רחוב כבוי השבוע, חשוך מאוד בלילה', addr: 'שדרות רוטשילד 18', lat: 32.0636, lng: 34.7733, urgency: client_1.Urgency.NORMAL, status: client_1.IssueStatus.ASSIGNED },
        { cat: 2, desc: 'ערימת פסולת ליד הפח ברחוב דיזנגוף', addr: 'רחוב דיזנגוף 99', lat: 32.0778, lng: 34.7744, urgency: client_1.Urgency.NORMAL, status: client_1.IssueStatus.IN_PROGRESS },
        { cat: 4, desc: 'מדרכה שבורה ומסוכנת, סכנה לקשישים', addr: 'רחוב אלנבי 30', lat: 32.0694, lng: 34.7688, urgency: client_1.Urgency.HIGH, status: client_1.IssueStatus.NEW },
        { cat: 7, desc: 'שלט נפל בגינה הציבורית', addr: 'רחוב בן יהודה 5', lat: 32.0834, lng: 34.7694, urgency: client_1.Urgency.LOW, status: client_1.IssueStatus.RESOLVED },
        { cat: 3, desc: 'ברזל בולט מהקרקע ליד גן ילדים', addr: 'רחוב ארלוזורוב 80', lat: 32.0876, lng: 34.7836, urgency: client_1.Urgency.CRITICAL, status: client_1.IssueStatus.IN_PROGRESS },
        { cat: 6, desc: 'רמזור מהבהב, לא עובד כראוי', addr: 'צומת קפלן-איבן גבירול', lat: 32.0728, lng: 34.7858, urgency: client_1.Urgency.HIGH, status: client_1.IssueStatus.ASSIGNED },
        { cat: 5, desc: 'הצפה קלה לאחר גשם, בור ניקוז סתום', addr: 'רחוב חיים ברלב 12', lat: 32.0952, lng: 34.7901, urgency: client_1.Urgency.NORMAL, status: client_1.IssueStatus.NEW },
        { cat: 8, desc: 'רכב נטוש כבר חודש ואף אחד לא מפנה', addr: 'רחוב פינסקר 44', lat: 32.0682, lng: 34.7701, urgency: client_1.Urgency.LOW, status: client_1.IssueStatus.PENDING_VERIFICATION },
        { cat: 9, desc: 'גרפיטי על קיר בניין ציבורי', addr: 'רחוב לילינבלום 20', lat: 32.0612, lng: 34.7711, urgency: client_1.Urgency.LOW, status: client_1.IssueStatus.NEW },
        { cat: 0, desc: 'שקיעת כביש ליד צומת, מסוכן בנסיעה', addr: 'רחוב ז\'בוטינסקי 7', lat: 32.0898, lng: 34.7928, urgency: client_1.Urgency.CRITICAL, status: client_1.IssueStatus.ASSIGNED },
        { cat: 2, desc: 'גזם לאחר גיזום עצים, חוסם מדרכה', addr: 'שדרות שאול המלך 50', lat: 32.0806, lng: 34.7875, urgency: client_1.Urgency.NORMAL, status: client_1.IssueStatus.RESOLVED },
    ];
    ...
    const issues = await Promise.all(issueSeeds.map((seed, i) => prisma.issueReport.create({
        data: {
            tenantId: telAviv.id,
            ...
            resolvedAt: seed.status === client_1.IssueStatus.RESOLVED ? new Date() : null,
    ...
    const historyEntries = issues.flatMap((issue) => {
        const base = [{ issueId: issue.id, fromStatus: null, toStatus: client_1.IssueStatus.NEW, createdAt: issue.createdAt }];
        if (issue.status !== client_1.IssueStatus.NEW) {
            base.push({ issueId: issue.id, fromStatus: client_1.IssueStatus.NEW, toStatus: issue.status, createdAt: new Date() });
        }
        return base;
    });
